...
| <span style="color:#1565c0">`SideSensorUpdated(leftBlocked)`</span> | Left Sensor | <span style="color:#1565c0">회피 중 좌측 장애물 상태만 controller에 전달한다 (CHG-001: 우측 센서 제거, `rightBlocked` 파라미터 삭제).</span> | FR-008, FR-009, FR-010, FR-011, FR-012, FR-013 |
...
<!-- [CHG-001] SideSensorUpdated: rightBlocked 파라미터 제거, 좌측 단일 센서 기반 회피 -->
### <span style="color:#1565c0">`SideSensorUpdated(leftBlocked)`</span>

| Field | Contract |
| --- | --- |
| Preconditions | `state=AvoidingObstacle` |
| Postconditions | <span style="color:#1565c0">left clear -> motor `TurnLeft`; left blocked -> motor `TurnRight` (우측 센서가 없으므로 좌측이 막히면 반대쪽인 우측으로 회피를 시도한다).</span> <span style="color:#c62828">~~left clear -> motor left, right clear -> motor right, both blocked -> motor backward~~</span> |
| Policy | <span style="color:#1565c0">좌측 센서 하나로만 판단한다. 좌측이 비면 좌회전, 막히면 우회전한다. 우측 가용 여부는 우회전 후 전방 센서로 간접 확인한다.</span> <span style="color:#c62828">~~left side has priority over right side~~</span> |
...
| --- | --- |
| Preconditions | `state=AvoidingObstacle` |
| Postconditions after `TurnLeft` | <span style="color:#1565c0">normal left-avoidance turn (`right_turn_attempted_ == false`) -> resume `CleaningForward` (motor forward, cleaner on). Dead-end recovery turn (`right_turn_attempted_ == true`) -> motor `Backward` into the open cell behind, then clear the flag. (Option C)</span> |
| Postconditions after `TurnRight` | <span style="color:#1565c0">request front check; front clear -> `CleaningForward`; front still blocked (= dead-end) -> motor `TurnLeft` to restore the original heading first (avoids reversing into the left obstacle), `Backward` is issued only after that turn completes. (Option C)</span> |
| Postconditions after `Backward` | <span style="color:#1565c0">restart avoidance: request `SideSensorUpdated(leftBlocked)` again.</span> |
...
| Main Flow | <span style="color:#1565c0">Controller cancels power-up timer, turns cleaner off, enters `AvoidingObstacle`, checks the **left sensor only**, and if clear turns left, then resumes forward cleaning.</span> <span style="color:#c62828">~~checks left/right sensors, turns toward clear side~~</span> |
| <span style="color:#1565c0">Alternative A</span> | <span style="color:#1565c0">Left blocked: turn right (no right sensor; the opposite side is the likely opening), then re-check the front sensor.</span> <span style="color:#c62828">~~Left blocked and right clear: turn right.~~</span> |
| <span style="color:#1565c0">Alternative B</span> | <span style="color:#1565c0">After turning right, front clear (right was open): resume forward cleaning without backing up. Front still blocked (front + left + right all blocked, a dead-end): first `TurnLeft` to restore the original heading (so the robot does not reverse into the left obstacle it just turned away from), then move `Backward` into the open cell behind, then restart avoidance from the left-sensor check. (Option C)</span> <span style="color:#c62828">~~Left and right blocked: move backward repeatedly until one side is clear, then turn toward clear side.~~</span> |
...
| FR-008, FR-009 | <span style="color:#1565c0">`SideSensorUpdated(leftBlocked)` checks the left sensor only; left clear -> `TurnLeft`. (CHG-001)</span> |
| FR-010, FR-011 | <span style="color:#1565c0">left blocked -> `TurnRight`; after the right turn, the front sensor is re-checked. (CHG-001)</span> |
| FR-012, FR-013 | <span style="color:#1565c0">after right turn: front clear -> resume forward (no backward); front still blocked (dead-end) -> `TurnLeft` to restore heading, then `Backward`, then restart avoidance. (CHG-001, Option C)</span> <span style="color:#c62828">~~both sides blocked leads to repeated backward motion until a side is clear.~~</span> |
...
| Backward-until-clear needs an event that tells the controller when to re-check sides. | Use `MotionCompleted(Backward)` followed by <span style="color:#1565c0">`SideSensorUpdated(leftBlocked)` (CHG-001: left sensor only)</span>. |
| <span style="color:#2e7d32">[CHG-001] Right sensor removed but preliminary requirements still require backward only when front+left+right are all blocked, without backing up unnecessarily.</span> | <span style="color:#2e7d32">When left is blocked, turn right first (no preemptive backward). The front sensor then re-checks the new heading: if clear, the right side was open and we resume without backing up; if still blocked, all three directions are blocked (dead-end). This uses only the front + left sensors and triggers backward exactly in the all-blocked case.</span> |
| <span style="color:#2e7d32">[CHG-001/Option C] After a right turn the robot's heading has changed, so an immediate `Backward` would reverse into the left obstacle it just turned away from (collision behind).</span> | <span style="color:#2e7d32">On a dead-end, the controller first issues `TurnLeft` to restore the original heading; only then is `Backward` issued, reversing into the cell the robot came from (always open). `right_turn_attempted_` distinguishes this recovery `TurnLeft` (-> Backward) from a normal left-avoidance `TurnLeft` (-> resume forward).</span> |
