...
| FR-015 | 전진 중 전방 먼지가 감지되면 `rvc_controller`는 cleaner의 흡입력을 5초 동안 증가시켜야 한다. |
| FR-016 | 흡입력 증가 후 5초가 지나면 `rvc_controller`는 cleaner의 흡입력을 정상화해야 한다. |
...
| NFR-002 | 먼지 감지 후 흡입력 증가 시간은 5초를 기준으로 하며, 타이머 오차는 구현 단계에서 명시적으로 관리되어야 한다. |
...
| Supporting Actors | Cleaner, Timer |
| Trigger | 전진 중 전방 먼지가 감지된다. |
| Preconditions | `rvc_controller` 전원이 켜져 있고 Motor가 전진 중이며 Cleaner가 켜져 있다. |
| Postconditions | 5초 동안 흡입력이 증가한 뒤 정상 흡입력으로 복귀한다. |
| Main Success Scenario | 1. Dust Sensor가 먼지를 감지한다.<br>2. `rvc_controller`가 Cleaner에 `PowerUp` 명령을 보낸다.<br>3. `rvc_controller`가 5초 타이머를 시작한다.<br>4. 5초가 경과한다.<br>5. `rvc_controller`가 Cleaner에 `On` 명령을 보내 정상 흡입력으로 복귀한다. |
| Extensions | 3a. 5초가 지나기 전에 전방 장애물이 감지되면 증가 시간을 저장하지 않는다.<br>3a1. `rvc_controller`가 Cleaner에 `Off` 명령을 보낸다.<br>3a2. UC-003을 수행한다. |
